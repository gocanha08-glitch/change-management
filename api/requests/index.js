...
module.exports = async (req, res) => {
        ...
        if (req.method === 'POST') {
                  ...
                  try {
                    ...
                    const [seqRow] = await sql`SELECT nextval('sa_id_seq') AS nextval`;
                              const year = new Date().getFullYear();
                              const num = String(seqRow.nextval).padStart(3, '0');
                              sa.id = `SA-${num}/${year}`;
                              sa.status = 'aberta';
                              sa.createdBy = user.id;

                    // Passar objeto direto para JSONB (nao usar JSON.stringify)
                    await sql`
                    ...
                    const sgqUsers = await sql`
                            SELECT u.email FROM users u
                              ...
                              console.error('POST request error:', err.message, err.stack);
